...
async def run_agent(languages: str = "typescript,python", limit: int = 5):
    """Run backend-owned discovery and stream safe execution events via SSE."""
    if db is None:
        raise HTTPException(status_code=503, detail="Database not initialized")
...
async def discover_repos(languages: str = "typescript,python", limit: int = 5) -> Dict[str, Any]:
    ...
    try:
        client, store = GitHubClient(), FirestoreClient()
        found = client.search_trending(languages=[x.strip() for x in languages.split(',') if x.strip()], per_page=max(1, min(limit, 30)))
        saved, skipped = [], 0
        for repo in found:
            url = repo.get('url') or ''
            if not url: continue
            if store.check_duplicate(url): skipped += 1; continue
            doc_id = store.create_repo({'github_url': url, 'source': 'github_trending', 'raw_name': repo['name'], 'raw_description': repo.get('description',''), 'stars': repo['stars'], 'topics': repo.get('topics',[]), 'readme_url': repo.get('readme_url',''), 'status': 'pending_analysis', 'created_at': firestore.SERVER_TIMESTAMP})
            saved.append({'id': doc_id, 'name': repo['name'], 'url': url})
        return {'status':'success','count':len(saved),'github_found':len(found),'duplicates_skipped':skipped,'repos':saved}
    except Exception as e:
        log_safe_error('discover_repos', e); raise HTTPException(status_code=500, detail='Discovery failed')
...
async def analyze_repo(repo_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        data, readme = doc.to_dict(), GitHubClient().fetch_readme(doc.to_dict().get('github_url',''))
        if not readme:
        ...
        text=(genai.Client(api_key=Config.GEMINI_API_KEY).models.generate_content(model=Config.GEMINI_MODEL, contents=prompt).text or '').strip().replace('```json','').replace('```','').strip()
        if not text: raise HTTPException(status_code=502, detail='Empty analysis response')
        ...
        log_safe_error('analyze_repo',e,{'repo_id':repo_id}); raise HTTPException(status_code=500, detail='Analysis failed')
...
async def publish_linkedin(repo_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        post_text=(genai.Client(api_key=Config.GEMINI_API_KEY).models.generate_content(model=Config.GEMINI_MODEL, contents=prompt).text or '').strip()
...
async def publish_devto(repo_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        article=(genai.Client(api_key=Config.GEMINI_API_KEY).models.generate_content(model=Config.GEMINI_MODEL, contents=prompt).text or '').strip()
        if not article: raise HTTPException(status_code=502, detail='Empty Dev.to article response')
